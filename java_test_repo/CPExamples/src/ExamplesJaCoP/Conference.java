 ...
 * 
 * It solves a simple conference example problem, where 11 different sessions 
 * must be scheduled according to the specified constraints in the 2 days conference.
 *
 */
...
public class Conference extends Example {
	...
	public void model() {
		...
		IntVar[] sessions = new IntVar[11];
                ...
                //each day is devided for two halfs, so 4 periods
		for (int i = 0; i < 11; i++) {
		...
		//see javadoc for more constructors definitions
		store.impose(new Cumulative(sessions, durations, resources, three));
                
//                IntVar suma = new IntVar(store, "suma", 1, 50);
//                cost = suma;
//                store.impose(new Sum(sessions, cost));
//                vars.add(suma);
